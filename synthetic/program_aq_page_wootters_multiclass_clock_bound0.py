...
Estimate whether remaining local-window failures are close to a multiclass
clock-state distinguishability proxy. This freezes the A10 event-damped bridge
and adds a local pretty-good-measurement style bound over path-control
candidate records.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("aq_page_wootters_multiclass_clock_bound0"))
...
def clock_bound_metrics(alice, bob, path_nulls, query, rel_match, control_matches, gains):
    ...
    class_similarities = jnp.concatenate((true_similarity[None, :], null_stack), axis=0)
    eps = 1e-6
    inverse_similarity_success = (1.0 / (eps + 1.0 - true_similarity)) / jnp.sum(
        1.0 / (eps + 1.0 - class_similarities), axis=0
    )
    softmax_success = jax.nn.softmax(8.0 * class_similarities, axis=0)[0]
    gram_stack = jnp.concatenate((true_state[None, :, :], jnp.stack([
        normalized_rows(path_nulls[name])[control_matches[name]]
        for name in control_matches
    ])), axis=0)
    local_gram = jnp.einsum("cnf,dnf->ncd", gram_stack, gram_stack)
    local_eval = jnp.linalg.eigvalsh(local_gram)
    local_eff_rank = (jnp.sum(local_eval, axis=1) ** 2) / jnp.maximum(
        jnp.sum(local_eval * local_eval, axis=1), 1e-8
    )
    multiclass_effrank_bound = local_eff_rank / local_gram.shape[1]
    ...
    return {
        ...
        "pgm_inverse_similarity_success_mean": float(jnp.mean(inverse_similarity_success)),
        "pgm_inverse_similarity_success_median": float(jnp.median(inverse_similarity_success)),
        "softmax_multiclass_success_mean": float(jnp.mean(softmax_success)),
        "multiclass_effrank_bound_mean": float(jnp.mean(multiclass_effrank_bound)),
        ...
        "pgm_normalized_window_score": float(jnp.mean(observed_success) / jnp.maximum(jnp.mean(inverse_similarity_success), 1e-6)),
...
def aggregate(results):
    ...
    for key, rows in grouped.items():
        out[key] = {
            ...
            "median_pgm_success": float(np.median([r["metrics"]["pgm_inverse_similarity_success_mean"] for r in rows])),
            "median_pgm_normalized_window_score": float(np.median([r["metrics"]["pgm_normalized_window_score"] for r in rows])),
            "median_softmax_success": float(np.median([r["metrics"]["softmax_multiclass_success_mean"] for r in rows])),
            "median_effrank_bound": float(np.median([r["metrics"]["multiclass_effrank_bound_mean"] for r in rows])),
...
def main():
    ...
    print(f"AQ-PAGE-WOOTTERS-DLINOSS-MULTICLASS-CLOCK-BOUND-0 backend={backend} configs={len(configs)}", flush=True)
    results = []
    for index, (seed, noise, hidden, context, operator, loss) in enumerate(configs):
        ...
        m = result["metrics"]
        print(f"[RESULT] gain={m['dlinoss_relational_gain']:.6f} minus_ridge={m['dlinoss_minus_ridge']:.6f} windows={m['fraction_windows_beating_best_null']:.3f} pair_bound={m['helstrom_success_bound_mean']:.3f} pgm={m['pgm_inverse_similarity_success_mean']:.3f} pgm_norm={m['pgm_normalized_window_score']:.3f}", flush=True)
    payload = {
        "program": "AQ-PAGE-WOOTTERS-DLINOSS-MULTICLASS-CLOCK-BOUND-0",
        "backend": backend,
    ...
    output = args.out_dir / "multiclass_clock_bound0_results.json"

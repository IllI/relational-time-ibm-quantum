...
Confirmation grid for the winning A10 event-damped D-LinOSS setup. This freezes
the generator/path/operator shape and compares only uniform versus event-weighted
training over more seeds and noise levels.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--losses", nargs="*", default=["uniform", "event_weighted"])
    parser.add_argument("--epochs", type=int, default=350)
    parser.add_argument("--learning-rate", type=float, default=3e-3)
    parser.add_argument("--bootstrap-samples", type=int, default=400)
    parser.add_argument("--out-dir", type=Path, default=Path("aq_page_wootters_a10_event_damped_confirm0"))
...
def main():
    ...
    print(f"AQ-PAGE-WOOTTERS-DLINOSS-A10-EVENT-DAMPED-CONFIRM-0 backend={backend} configs={len(configs)}", flush=True)
    results = []
    for index, (seed, noise, hidden, context, operator, loss) in enumerate(configs):
        ...
        m = result["metrics"]
        print(f"[RESULT] gain={m['dlinoss_relational_gain']:.6f} minus_ridge={m['dlinoss_minus_ridge']:.6f} windows={m['fraction_windows_beating_best_null']:.3f} ext={m['dlinoss_relational_minus_external_gain']:.6f} ci_low={m['ci_lower']:.6f}", flush=True)
    payload = {
        "program": "AQ-PAGE-WOOTTERS-DLINOSS-A10-EVENT-DAMPED-CONFIRM-0",
        "backend": backend,
    ...
    output = args.out_dir / "a10_event_damped_confirm0_results.json"
